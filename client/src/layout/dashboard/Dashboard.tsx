...
export default function Dashboard() {
    ...
    const handleCurrencyChange = (value: AvailableCurrency) => {
    ...
    return (
        <div className="px-4 py-4 h-[calc(100vh-3.5rem)] overflow-hidden scrollbar-thin scrollbar-thumb-gray-400 dark:scrollbar-thumb-gray-600 scrollbar-track-transparent hover:scrollbar-thumb-gray-500 dark:hover:scrollbar-thumb-gray-500">
            <Tabs value={tabValue} onValueChange={handleTabChange} className="w-full gap-2">
                ...
                <div className="flex items-center gap-4">
                    <OverviewCards type="receivable" align="stretch" currency={selectedCurrency} />
                    <Separator orientation="vertical" className="!h-20 w-full" />
                    <OverviewCards type="payable" align="stretch" currency={selectedCurrency} />
                </div>
                <TabsContent value="receivable">
                    <CustomerTable type="receivable" data={receivableCustomers} currency={{ state: selectedCurrency, onChange: handleCurrencyChange }} />
                </TabsContent>
                <TabsContent value="payable">
                    <CustomerTable type="payable" data={payableCustomers} currency={{ state: selectedCurrency, onChange: handleCurrencyChange }} />
                </TabsContent>
            </Tabs>
        </div>
    )
