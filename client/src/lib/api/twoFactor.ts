...
export class TwoFactorApi {
     ...
     */
    static async verifySetup(setupToken: string, code: string): Promise<TwoFactorVerifySetupResponse> {
        try {
     ...
     */
    static async verify(code: string, tempToken: string): Promise<TwoFactorVerifyResponse> {
        try {
     ...
     */
    static async useRecoveryCode(code: string, tempToken: string): Promise<TwoFactorRecoveryResponse> {
        try {
            const response = await instance.post(
                "/2fa/recovery",
                { code },
     ...
     */
    static async disable(password: string, code: string): Promise<{ success: boolean; message: string }> {
        try {
