...
export async function exportCustomerStatementPDF(statement: CustomerStatement) {
    ...
    let lineY = nameTopY;
    lines.slice(0, 5).forEach(l => { // cap at 5 lines to avoid pushing too far
    ...
    // Summary three columns below header separator
    ...
    const metrics = [
        { label: 'Toplam Borç', value: formatCurrency(customer.total_debt || 0), color: [180, 30, 30] },
        { label: 'Toplam Ödeme', value: formatCurrency(customer.total_payments || 0), color: [25, 135, 84] },
        { label: 'Kalan', value: formatCurrency(customer.remaining_debt || 0), color: [25, 70, 170] },
    ];
    doc.setFontSize(11);
    metrics.forEach((m, i) => {
        ...
        doc.text(m.value, x, boxY + 24);
        doc.setFontSize(11);
    });
    // reset color
    doc.setTextColor(0, 0, 0);
    // Reduce spacer before 'Borçlar' heading
    cursorY = boxY + 34; // was +40
    ...
    autoTable(doc, {
        ...
        head: [['Tarih', 'Fatura No', 'Tutar', 'KDV', 'Toplam']],
        ...
        didDrawPage: (data: { pageNumber: number }) => {
            ...
            doc.text(`Sayfa ${data.pageNumber} / ${pageCount}`,
