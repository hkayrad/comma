...
export class DebtRepository {
	...
	async findAllWithSummary(
        ...
        const colMap: Record<string, string> = {
            customer_name: "c.name", amount: "d.amount", vat: "d.vat",
            total: "(d.amount + d.vat)", discount: "d.discount", withholding: "d.withholding",
            currency: "d.currency", exchange_rate: "d.exchange_rate",
            total_in_try: "((d.amount + d.vat) * d.exchange_rate)",
            issue_date: "d.issue_date", due_date: "d.due_date",
            invoice_no: "d.invoice_no", description: "d.description",
        };
        ...
        const countQuery = `SELECT COUNT(*) as count FROM ${this.domain}_debts d JOIN ${this.domain}_customers c ON d.customer_id = c.id AND c.company_id = d.company_id ${whereClause}`;
	...
	async getTotals(companyId: UUID, currency: string): Promise<Totals | null> {
        const query = `SELECT COALESCE(d.total_in_try, 0) AS total_debts, COALESCE(p.total_in_try, 0) AS total_payments, COALESCE(d.total_in_try, 0) - COALESCE(p.total_in_try, 0) AS remaining_debt FROM (SELECT ? AS company_id) AS input LEFT JOIN vw_${this.domain}_total_debt_by_company d ON d.company_id = input.company_id LEFT JOIN vw_${this.domain}_total_payments_by_company p ON p.company_id = input.company_id;`;
        const result = (await sequelize.query(query, { replacements: [companyId], type: QueryTypes.SELECT })) as Totals[];
