...
export default class ReceivableCustomersService {
	...
	static async GetStatement(customerId: UUID, companyId: UUID) {
		...
		try {
          ...
          INNER JOIN receivable_customers pc ON d.customer_id = pc.id AND d.company_id = pc.company_id
          WHERE d.customer_id = ?
            AND d.company_id = ?
          ...
          INNER JOIN receivable_customers pc ON p.customer_id = pc.id AND p.company_id = pc.company_id
          WHERE p.customer_id = ?
            AND p.company_id = ?
