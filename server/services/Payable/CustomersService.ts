...
export default class PayableCustomersService {
    static async Create(customer: any) {
        ...
        try {
            ...
            const query = `
            INSERT INTO payable_customers (name, phone, is_company, tax_number, tax_office, mersis_no, email, address)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING id
            `;
    ...
    static async GetAll() {
        ...
        try {
            ...
            Logger.info("Retrieved payable_customers:", result);

            if (result.length === 0)
                return ApiResponse.error("No payable_customers found");

            return ApiResponse.success(result, "Customers retrieved successfully");
        } catch (error) {
            Logger.error("Failed to retrieve payable_customers:", error);
            return ApiResponse.error("Failed to retrieve payable_customers");
        } finally {
            if (conn) conn.release();
        }
    }
    ...
    static async GetIdAndName() {
        ...
        try {
            const query = `
            SELECT id, name FROM payable_customers
            `;
            ...
            Logger.error('Error retrieving payable_customers:', error);
            return ApiResponse.error("Error retrieving payable_customers");
        } finally {
            if (conn) conn.release();
        }
    }
    ...
    static async Delete(id: string) {
        ...
        try {
            ...
            const query = `
            DELETE FROM payable_customers WHERE id = ?
            `;
