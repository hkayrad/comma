...
export default class PayableDebtsService {
	...
	static async CreateBatch(debts: DebtDto[], userId: UUID, companyId: UUID) {
		Logger.info("[PayableDebts] Creating debts batch", { companyId, count: debts.length, userId });
		...
		return await sequelize.transaction(async (t) => {
			...
			Logger.info("[PayableDebts] Debts batch created successfully", { companyId, count: result.length });
			return result;
		});
	}

	static async GetAll(companyId: UUID, page: number, limit: number, sorting: SortItem[] = [], filters: FilterItem[] = []) {
		Logger.debug("[PayableDebts] Fetching all debts", { companyId, page, limit });
	...
	static async GetTotals(companyId: UUID, currency: string) {
		...
		Logger.debug("[PayableDebts] Totals fetched successfully", { companyId, currency });
		return totals;
	}
	...
	static async DeleteBatch(ids: UUID[], userId: UUID, companyId: UUID) {
		Logger.info("[PayableDebts] Deleting debts batch", { companyId, count: ids?.length, userId });
		...
		return await sequelize.transaction(async (t) => {
			...
			Logger.info("[PayableDebts] Debts batch deleted successfully", { companyId, count: deletedCount });
			return deletedCount;
		});
	}
	...
	static async Restore(id: UUID, userId: UUID, companyId: UUID) {
		Logger.info("[PayableDebts] Restoring debt", { debtId: id, companyId, userId });
		if (!id) throw new ValidationError("Debt ID is required");
		...
		Logger.info("[PayableDebts] Debt restored successfully", { debtId: id, companyId });
	}
