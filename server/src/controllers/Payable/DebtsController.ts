...
router.post("/debts/bulk-delete", validate(bulkDeleteSchema), asyncHandler(async (req: Request, res: Response) => {
	const { ids } = req.body;
	const { id: userId, companyId } = req.user;
	Logger.info("[PayableDebtsController] Bulk delete debts request", { companyId, count: ids.length });

	const deletedCount = await PayableDebtsService.DeleteBatch(ids, userId, companyId);
...
router.post("/debts/:id/restore", asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
	const { id } = req.params;
	const { id: userId, companyId } = req.user;
	Logger.info("[PayableDebtsController] Restore debt request", { debtId: id, companyId });

	await PayableDebtsService.Restore(id, userId, companyId);
