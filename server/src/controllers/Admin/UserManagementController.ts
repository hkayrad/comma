...
router.get("/:id", asyncHandler(async (req: Request, res: Response) => {
	...
	const data = await UserManagementService.GetById(req.params.id as string);
	res.json({ success: true, data });
}));
...
router.put("/:id", authRateLimiter, asyncHandler(async (req: Request, res: Response) => {
	...
	const data = await UserManagementService.Update(req.params.id as string, req.body, req.user.id);
...
router.post("/:id/restore", authRateLimiter, asyncHandler(async (req: Request, res: Response) => {
	...
	await UserManagementService.Restore(req.params.id as string);
...
router.post("/:id/reset-password", authRateLimiter, asyncHandler(async (req: Request, res: Response) => {
	...
	await UserManagementService.ResetPassword(req.params.id as string, password);
