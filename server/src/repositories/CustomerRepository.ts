...
export class CustomerRepository {
	...
	async getStatement(
		...
		let debtsQuery = `
            ...
            d.currency,
            d.exchange_rate,
            (d.amount + d.vat - COALESCE(d.discount, 0) - COALESCE(d.withholding, 0)) AS total,
            ((d.amount + d.vat - COALESCE(d.discount, 0) - COALESCE(d.withholding, 0)) * d.exchange_rate) AS total_in_try,
            d.description,
            d.issue_date,
            d.created_at
